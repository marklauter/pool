...
**Resolved nits:** `IsNotExpired()` (gone with `LeaseRequest`); `[MethodImpl(AggressiveInlining)]` dropped from `Release`; `ItemsAvailable => pool?.Count ?? 0` simplified to `pool.Count` after the ctor reorder. **`DateTime.UtcNow` / wall-clock everywhere (was M4) — fully resolved:** the pool takes an injected `TimeProvider` (optional trailing ctor param, defaults to `TimeProvider.System`, so the contract is unchanged and DI keeps resolving it). Every clock read now flows through it: idle stamping/eviction uses `timeProvider.GetUtcNow()` (`PoolItem.IdleSince` is a `DateTimeOffset`); the **lease timeout** is driven by `new CancellationTokenSource(leaseTimeout, timeProvider)` linked with the caller's token (routing around `SemaphoreSlim.WaitAsync` having no `TimeProvider` overload), translated to `TaskCanceledException` to preserve the v6 contract; the **preparation timeout** uses `new CancellationTokenSource(preparationTimeout, timeProvider)`; and the lease/preparation **metric timers** use `timeProvider.GetTimestamp()` + `GetElapsedTime()` instead of `Stopwatch` (the `System.Diagnostics` using is gone). Deterministically tested via `FakeTimeProvider`: `Idle_Item_Past_Timeout_Is_Disposed_And_Fresh_One_Served`, `Lease_Times_Out_Deterministically_When_Capacity_Stays_Saturated`, `Preparation_Times_Out_Deterministically_When_Strategy_Stalls` — no `Task.Delay`/wall-clock waits. See `pool-tests.findings.md` M4.
